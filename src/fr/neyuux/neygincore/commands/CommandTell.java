...
public class CommandTell implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String alias, String[] args) {
		...
		if (args.length == 0) {
			sender.sendMessage(main.getPrefix() + "§8§l» " + help);
		} else {
			...
			if (args[0].equalsIgnoreCase("help")) sender.sendMessage(main.getPrefix() + "§8§l» " + help);
				...
				if (p == null) {
					...
					if (sender instanceof Player) main.lastMessages.put(p, (Player) sender);
					sender.sendMessage("§5[§bEnvoyé à §a§l" + p.getName() + "§5] §8§l» §e" + msg);
					if (sender instanceof Player) main.lastMessages.put((Player) sender, p);
