		...
		if (sender instanceof Player) {
			...
			if (CommandTell.lastMessages.containsKey(player)) {
				...
				if (args.length > 0) {
					...
					p.sendMessage("§5[§bRe§u de §a§l" + player.getName() + "§5] §8§l§ §e" + bc);
					CommandTell.lastMessages.put(p, player);
					player.sendMessage("§5[§bEnvoy§ § §a§l" + p.getName() + "§5] §8§l§ §e" + bc);
				} else
					player.sendMessage(main.getPrefix() + "§8§l§ " + help);
			} else
				player.sendMessage(main.getPrefix() + "§8§l§ §cVous n'avez personne § qui r§pondre !");
		} else
			sender.sendMessage(main.getPrefix() + "§8§l§ §cVous n'§tes pas un joueur !");
